...
describe('Koa Connect Cloud Health test suite', function () {
    ...
    it('Liveness returns 200 OK and STARTING on startup check starting', async function () {
        ...
        validateResult(ctx, 200, "{\"status\":\"STARTING\",\"checks\":[{\"name\":\"startup\",\"state\":\"STARTING\",\"data\":{\"reason\":\"\"}}]}");
    });
    ...
    it('Readiness returns 503 Unavailable and STARTING on startup check starting', async function () {
        const cloudHealth = new HealthChecker();
        cloudHealth.registerStartupCheck(
            new StartupCheck("startup", () => new Promise<void>(function (resolve) {
                resolve();
